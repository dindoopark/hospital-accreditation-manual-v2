...
- **웹**: https://dindoopark.github.io/hospital-accreditation-manual-v2/
  (검색엔진 수집은 막아 두었습니다. 처음 열 때 PIN 6자리를 묻습니다)
...
- 한 번 맞게 넣으면 그 기기·브라우저에서는 다시 묻지 않습니다 (「이 기기에서 다시 묻지 않기」를 해제하면 브라우저를 닫을 때까지만).
- PIN 바꾸기: 사이트를 열고 개발자도구 콘솔에서 `PinLock.hash('새PIN6자리')` 실행 → 나온 값을
  `docs/index.html` 의 `data-hash` 에 넣습니다. 이미 열어 둔 기기도 새 PIN 을 다시 묻습니다.
